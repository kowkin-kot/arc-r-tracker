...
// 1. Fix projectsList
// I'll re-insert a clean projectsList at the top
const projectsListStart = content.indexOf('export const projectsList');
...
// Boundary between itemsList and blueprintsList
// I see zipline at 8856, and angled_grip_ii_blueprint at 8873.
const ziplineIndex = content.indexOf('"id": "zipline"');
if (ziplineIndex === -1) {
  console.error("Could not find zipline");
  process.exit(1);
}

const angledGripIndex = content.indexOf('"id": "angled_grip_ii_blueprint"');

const blueprintsDeclaration = `];

export const blueprintsList: Blueprint[] = [
`;

// Reconstruct
let newContent = content.substring(0, projectsListStart) + safeProjectsList;

// From itemsList start tozipline's end
const itemsBody = content.substring(lastBraceBeforeItem, ziplineIndex);
// zipline object end
const ziplineEnd = content.indexOf('}', ziplineIndex) + 1;
const ziplineObject = content.substring(ziplineIndex - 2, ziplineEnd); // catch the {

newContent += itemsBody + ziplineObject + blueprintsDeclaration;

// From angledGrip to the end
const lastItemStart = content.lastIndexOf('{', angledGripIndex);
newContent += content.substring(lastItemStart);

// Clean up double ] at the end
newContent = newContent.replace(/\]\s*\]\s*;\s*$/g, '];');

fs.writeFileSync(filePath, newContent);
console.log("File fixed with boundary reconstruction");
