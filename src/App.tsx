  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (e.code === 'auth/invalid-email') {
        setAuthError('Неверный формат почты.');
        ...
        setAuthError('Эта почта уже занята.');
      } else if (e.code === 'auth/weak-password') {
        setAuthError('Слишком слабый пароль (минимум 6 символов).');
      } else {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {showLoginModal && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-full max-w-sm relative shadow-2xl">
              <button 
              ...
              {authError && (
                <div className="mb-4 bg-red-900/50 border border-red-500 text-red-200 text-sm px-3 py-2 rounded">
                  {authError}
                </div>
              )}
              <form onSubmit={handleAuth} className="flex flex-col gap-3">
                ...
                <button 
                  type="submit"
                  className="mt-2 w-full py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-lg transition-colors"
                >
