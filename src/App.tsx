  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (e.code === 'auth/popup-blocked') {
        setAuthError('Всплывающее окно заблокировано. Откройте приложение в новой вкладке.');
      } else if (e.code === 'auth/unauthorized-domain') {
        setAuthError('Неавторизованный домен. Добавьте его в Firebase Console.');
      } else {
            ...
            <div className="flex items-center">
              {user ? (
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  {authError && (
                    <div className="absolute top-16 right-4 sm:right-auto sm:top-auto sm:mt-8 bg-red-900/90 border border-red-500 text-red-100 text-xs px-3 py-2 rounded shadow-xl z-50 animate-pulse text-right max-w-xs backdrop-blur-sm">
                      {authError}
                      <button className="ml-2 underline text-red-300" onClick={() => setAuthError(null)}>ОК</button>
                    </div>
                  )}
                </div>
              )}
